...
EXPECTED_FULL_CHECKPOINT_SHA = "f808fbd39adec37f499be05a7224c440e15e998c137b53c797f2733d9e5765ce8"
...
def build(*, full_checkpoint: Path, head_checkpoint: Path, calibration_summary: Path,
    ...
    if meta.get("updates") != 1400 or meta.get("validation_checkpoint_iteration") != 32500 or meta.get("train_windows") != 2052:
        raise ValueError("head metadata does not match frozen validation protocol")
    head_config = _jsonable_config(meta.get("feature_config"))
    if head_config != full_config:
        raise ValueError("validation/full feature_config mismatch")

    calibration = json.loads(calibration_summary.read_text(encoding="utf-8"))
    if calibration.get("gate") != "ADAPTIVE_GO":
        raise ValueError("adaptive calibration gate is not GO")
