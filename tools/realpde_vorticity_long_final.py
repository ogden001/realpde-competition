#!/usr/bin/env python3
"""Long-range matched validation for fixed Vorticity Supervision."""
...
START_UPDATE = 3000
FINAL_UPDATE = 15000
DEFAULT_MILESTONES = (3000, 4500, 6000, 9000, 12000, 15000)
EXPECTED_START = {
...
def validate_long_protocol(*, arm: str, batch_size: int, accumulation_steps: int,
                           seed: int, lr: float, start_update: int, final_update: int,
                           lambda_vort: float) -> None:
    if arm not in {"C0", "V1"}:
        raise ValueError("long-final arms must be C0 or V1")
    if batch_size != 8 or accumulation_steps != 1:
        raise ValueError("long-final requires physical batch=8 and no gradient accumulation")
    if seed != 20260901:
        raise ValueError("long-final requires seed 20260901")
    if lr != 1e-5:
        raise ValueError("long-final requires AdamW lr=1e-5")
    if start_update != 3000 or final_update != 15000:
        raise ValueError("long-final requires absolute continuation 3000 -> 15000")
    if arm == "V1" and lambda_vort != FIXED_LAMBDA_VORT:
        raise ValueError("V1 lambda_vort must equal the fixed R2 value exactly")
    if arm == "C0" and lambda_vort != 0.0:
        raise ValueError("C0 must use N2 only")


def relative_milestones(milestones: tuple[int, ...]) -> tuple[int, ...]:
    if tuple(milestones) != DEFAULT_MILESTONES:
        raise ValueError(f"milestones must be {DEFAULT_MILESTONES}")
    return tuple(update - START_UPDATE for update in milestones if update > START_UPDATE)


def window_order_digest(loader, seed: int) -> str:
    """Digest the first seeded RandomSampler permutation without touching training state."""
    order = list(iter(loader.sampler))
    payload = np.asarray(order, dtype=np.int64).tobytes()
    return hashlib.sha256(payload + str(seed).encode("ascii")).hexdigest()


def metric_improvement(c0: float, v1: float) -> float:
...
def _forward_zero_pressure(model, builder, x: torch.Tensor) -> torch.Tensor:
    ...
    pred = direct.forward(model, builder, x)
    pred = pred.clone()
    pred[..., 2] = 0.0
    return pred
...
def evaluate(model, builder, paths: list[Path], args: argparse.Namespace, device: torch.device,
    ...
    for x, y, _, _ in loader:
        ...
        pred = _forward_zero_pressure(model, args.builder, x)
        if device.type == "cuda":
            torch.cuda.synchronize()
...
def run(args: argparse.Namespace) -> None:
    ...
    validate_long_protocol(arm=args.arm, batch_size=args.batch_size, accumulation_steps=args.accumulation_steps,
                           seed=args.seed, lr=args.lr, start_update=START_UPDATE,
                           final_update=args.final_update, lambda_vort=args.lambda_vort)
    ...
    if device.type != "cuda":
        raise RuntimeError("long-final requires the target CUDA 3090; CPU fallback is forbidden")
    model, builder, config, payload = _load_model(args.kit_root, args.checkpoint, train_paths, device)
    ...
    # Probe uses a separate seeded loader; the training loader is freshly recreated afterward.
    ...
    # Each arm gets a new DataLoader and identical generator seed at continuation start.
    ...
    preflight = {
        ...
        "physical_batch": args.batch_size, "gradient_accumulation": False, "seed": args.seed,
        "window_protocol": "T_in=20,T_out=20,stride=20,seeded_train_loader_generator",
        "train_window_order_digest": train_order_digest,
        "probe_window_order_digest": probe_order_digest,
        ...
        "locked_final_accessed": False, "full_data_accessed": False, "sps_accessed": False,
    ...
    relative_targets = set(relative_milestones(tuple(args.milestones)))
    ...
    for relative in range(1, args.final_update - START_UPDATE + 1):
        ...
        optimizer.step()
        curve.append({"update": START_UPDATE + relative, "total": float(loss.detach().cpu()),
    ...
    runtime = {
        "arm": args.arm, "parameter_count": sum(p.numel() for p in model.parameters()),
        "added_parameter_count": 0, "training_wall_seconds": time.monotonic() - started,
        "peak_gpu_memory_allocated": peak_allocated, "peak_gpu_memory_reserved": peak_reserved,
        "inference_latency_final": aggregate[-1]["inference_time"], "adapter_output_norm": final_adapter_norm,
        "adapter_parameter_norm": 0.0,
    }
    ...
    metadata = {
        ...
        "lr": args.lr, "physical_batch": 8, "gradient_accumulation": False, "workers": args.workers,
        "lambda_vort": FIXED_LAMBDA_VORT if args.arm == "V1" else 0.0, "feature_config": vars(config),
        "train_trajectories": len(manifest["train"]), "dev_trajectories": len(json.loads(args.manifest.read_text())["dev"]),
        "milestones": list(args.milestones), "locked_final_accessed": False, "full_data_accessed": False,
