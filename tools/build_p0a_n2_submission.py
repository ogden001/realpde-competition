...
def validate_bounds(bound_abs: float | None, bound_rel: float | None) -> tuple[float, float] | None:
    if bound_abs is None and bound_rel is None:
        return None
    if bound_abs is None or bound_rel is None:
        raise ValueError("bound_abs and bound_rel must be supplied together")
    if bound_abs < 0 or bound_rel < 0:
        raise ValueError("bounds must be non-negative")
    return float(bound_abs), float(bound_rel)


def validate_checkpoint_for_submission(checkpoint: Path, *, required_iteration: int) -> dict:
    payload = torch.load(checkpoint, map_location="cpu", weights_only=False)
    if payload.get("iteration") != required_iteration:
        raise ValueError(
            f"checkpoint iteration must be {required_iteration}, got {payload.get('iteration')!r}"
        )
    if payload.get("feature_set") != "P0-A":
        raise ValueError("checkpoint is not marked P0-A")
    if payload.get("loss_weights") != N2_WEIGHTS:
        raise ValueError("checkpoint does not carry the frozen four-key N2 weights")
...
def package_file_inventory(root: Path) -> list[dict[str, int | str]]:
    ...
    for path in sorted(item for item in root.rglob("*") if item.is_file()):
        ...
        dependency = relative.startswith("rpde_baselines/") or relative.startswith("_vendor/einops/")
        allowed_source = dependency and path.suffix == ".py"
        allowed_license = dependency and path.name in LICENSE_FILENAMES
    ...
    missing = allowed_exact - {entry["path"] for entry in entries}
    if missing:
        raise ValueError(f"package inventory missing {sorted(missing)}")
    return entries


def submission_source(*, bound_abs: float | None, bound_rel: float | None) -> str:
    bounds = validate_bounds(bound_abs, bound_rel)
    return_bounds = bounds is not None
    abs_value = 0.0 if bounds is None else bounds[0]
    rel_value = 0.0 if bounds is None else bounds[1]
    return textwrap.dedent(f'''\
        ...
        _RETURN_BOUNDS = {return_bounds!r}
        _BOUND_ABS = {abs_value!r}
        _BOUND_REL = {rel_value!r}
        ...
        def predict(input_array, metadata=None):
            ...
            with torch.inference_mode():
                ...
                pred = model(_p0a(tensor, dx, dy).permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
                pred[..., 2] = 0.0
            prediction = pred.cpu().numpy().astype(np.float32, copy=False)
            if not _RETURN_BOUNDS:
                return prediction
            half_width = _BOUND_ABS + _BOUND_REL * np.abs(prediction)
            lower = (prediction - half_width).astype(np.float32, copy=False)
            upper = (prediction + half_width).astype(np.float32, copy=False)
            return {{"prediction": prediction, "lower": lower, "upper": upper}}
    ''')


def generate_submission_module(root: Path, *, bound_abs: float | None, bound_rel: float | None) -> None:
    root.mkdir(parents=True, exist_ok=True)
    (root / "submission.py").write_text(
        submission_source(bound_abs=bound_abs, bound_rel=bound_rel), encoding="utf-8"
    )
...
def copy_vendored_sources(source: Path, destination: Path) -> None:
    def ignore_non_runtime(_directory: str, names: list[str]) -> list[str]:
        ...
        for name in names:
            ...
            if name == "__pycache__" or (suffix and suffix != ".py" and name not in LICENSE_FILENAMES):
                ignored.append(name)
        return ignored
...
def build_submission(
    *, checkpoint: Path, required_iteration: int, kit_root: Path, out_dir: Path,
    experiment_id: str, git_commit: str, bound_abs: float | None, bound_rel: float | None,
) -> dict:
    if out_dir.exists():
    ...
    staging.mkdir()
    generate_submission_module(staging, bound_abs=bound_abs, bound_rel=bound_rel)
    minimal = {
    ...
    report = {
        ...
        "bounds": None if bounds is None else {"abs": bounds[0], "rel": bounds[1]},
        "zip": str(zip_path),
    ...
    (out_dir / "submission.zip.sha256").write_text(
        f"{report['zip_sha256']}  submission.zip\n", encoding="utf-8"
    )
    return report
...
def main() -> None:
    ...
    parser.add_argument("--bound-abs", type=float)
    parser.add_argument("--bound-rel", type=float)
