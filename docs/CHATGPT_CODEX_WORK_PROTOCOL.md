...
已有 GPU 任务**不构成新任务的默认阻塞条件**。无论新启动的是训练、评估还是纯推理，默认先判断**显存 / 主机内存是否有足够余量**，而不是根据 GPU utilization 判断能否启动。

核心规则：

1. **GPU utilization 100% 不是 BLOCK 条件。**
   - 已有任务可能已经把计算单元吃满，但仍有大量显存空闲；
   - 新任务可以与已有任务并行，只是吞吐可能下降；
   - 科学上首先关心是否能安全驻留显存、是否会 OOM、是否会破坏任务语义。

2. **启动前先看资源余量，而不是先等旧任务结束。**
   Codex 应优先检查：
   - 当前每个进程的 GPU memory；
   - GPU free memory；
   - 主机 RAM / swap；
   ...
   若有合理余量，直接并行启动，不因为“GPU 已有训练任务”而等待数小时。

3. **已有任务默认不动。**
   - 不暂停、不 kill、不重启已有正式任务；
   - 不修改已有任务的 batch、precision、模型、数据或训练配置；
   - 新任务自己适配资源。

4. **推理 / 评估任务的显存适配属于软约束。**
   在不改变预测值和评估口径的前提下，可按需降低：
   - eval batch size；
   - DataLoader workers；
   - prefetch / pin-memory 等纯运行参数。

   例如可按：
   `8 -> 4 -> 2 -> 1`
   逐级降低 eval batch，避免与已有训练产生 OOM。

5. **训练任务也允许共享 GPU 并行启动。**
   只要显存和主机内存允许，新训练任务可以与已有训练任务并行运行，不要求独占 GPU。

   但训练任务的科学配置仍受实验契约约束：
   - 若 effective batch / micro-batch / gradient accumulation 被实验协议冻结，则不得为了塞进显存而自行改变；
   - 只有被任务明确声明为运维软参数、且可证明不改变训练语义时，才可调整；
   - 不得擅自切换 FP32/BF16/FP16、MPS/MIG、模型宽度、输入尺寸、checkpointing 语义等方式来“挤显存”。

6. **若新任务发生 OOM，只处理新任务。**
   - 立即保护并保持已有任务继续运行；
   - 清理新任务自己的残留进程 / 显存；
   - 对推理 / 评估优先降低允许的 batch 等软参数后重试；
   - 对训练任务，只有在实验契约允许的范围内才能调整内存相关软参数；
   - 若在不改变科学语义的情况下仍无法共存，再选择其他 GPU 或等待资源释放；
   - 不得为了新任务去杀已有正式任务，除非用户 / Sol 明确授权。

7. **共享 GPU 会污染时间性能测量。**
   并行执行通常只影响 wall-clock / throughput，不应改变点预测任务的科学定义；但以下指标在共享 GPU 条件下默认不可作为正式比较依据：
   - runtime；
   - latency；
   - throughput；
   - SPS 中依赖推理耗时的部分；
   - 任何以 GPU 时间为实验变量的性能结论。

   如果任务目标包含正式 runtime / SPS / latency 对比，应：
   - 使用独占或可证明负载可比的 GPU 环境；或
   - 将共享 GPU 下的时间结果明确标记为 `INVALID_FOR_RUNTIME_COMPARISON`，只保留非时间指标。

8. **并行任务必须资源与目录隔离。**
   - 使用独立 PID / log / output dir；
   - 不共享可写 checkpoint 文件；
   - 不复用同一个临时输出路径；
   - 总主机内存仍遵守项目上限（当前默认 ≤48 GB）；
   - 记录启动时 GPU / RAM snapshot，方便后续解释吞吐波动或 OOM。
...
> **已有训练任务不是“占卡锁”；先看显存余量。能并行就并行，GPU utilization 不是阻塞条件。**
