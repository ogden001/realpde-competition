...
|---|---|---|---|---|---|
| **全合法时间窗训练（Dense-All）** | **改的只有 Train 的时间窗口采样方式。** 原 canonical 方案对每条轨迹按 `start=0,20,40,...` 取 Past20→Future20，Train50 共 2,052 windows；Dense-All 改成对长度为 `T` 的每条轨迹枚举**每一个合法起点 `s=0,1,...,T-40`**，每个样本仍是 `[s:s+20] → [s+20:s+40]`，不插值、不造 synthetic 数据。得到 40,488 windows（19.73×），每个 dense epoch 5,061 updates，30k ≈ 5.93 dense epochs；每个 epoch 对全部 dense windows 全局 shuffle。**Dev 完全不改**，仍是固定 stride=20 的 16 trajectories / 659 windows。DW-01 中初始化、P0-A、N2、AdamW `lr=1e-5`、batch=8、seed=20260901 全部保持不变，matched @7.5k 时唯一变量就是 window pool。 | 明确有效，是目前最强的数据利用策略之一 | 50/16 matched + horizon 分析 + SOTA-V2 线上 | [DW-01 实验记录](../experiments/dw01_dense_all_20260907/README_FOR_CHATGPT.md) · [Dense-All runner](../../tools/realpde_b1_p0a_n2.py) | 是 |
| **Past20 输入特征增强 + 多目标损失（P0-A + N2）** | **P0-A 是把 CNO 输入从原始 3 通道扩成总计 20 通道，不是“新增 20 通道”。** 原始 `u/v/p` 保留，再从 Past20 `u/v` 增加 17 个 causal/runtime-safe derived channels：①速度幅值；②`du/dx, du/dy, dv/dx, dv/dy`；③vorticity、`abs(vorticity)`、strain magnitude；④逐帧 `delta_u/delta_v`；⑤Past20 的 `u/v mean`、`u/v std`；⑥相对历史均值的 `u/v fluctuation`；⑦history TKE proxy。**不使用 P0-B 的 x/y/Re/AoA metadata。** 从 3-channel checkpoint warm-start 时，原 3 通道 lift 权重原样复制，新增 17 通道对应权重置零，保证 step-0 尽量保持原模型行为。N2 同时优化 `MSE + 0.05*TKE + 0.027514*Rel-L2 + 0.009757*MVPE`，即 pointwise、能量、相对误差和峰值误差一起约束。 | 明确有效，且长训仍持续改善 | CLEAN/competition validation + 线上历史 | [P0-A + N2 长训练](CHATGPT_HANDOFF_P0A_N2_VALIDATION_30900.md) · [P0-A 特征实现](../../tools/realpde_p0_features.py) | 是 |
| **延长训练至收敛平台区** | 这不是换模型，而是验证**训练预算本身**。固定 P0-A + N2、数据 split、模型和 optimizer 语义，从 0→4,100→6,800→10,300 继续恢复 checkpoint/optimizer state，一直训到 30,900；Dev 定期只做评估，不反向参与训练，也不读取 locked-final。结果显示 10.3k 仍明显欠训练：之后 best Rel-L2 / TKE / MVPE 分别还能再降约 9.16% / 4.39% / 12.56%。约 22k 后才进入宽平台/振荡区，因此后续不再用 5k/10k 的短训结果判断“是否收敛”。TKE 曲线更抖，所以 late stage 不是机械取 last，而是同时看 Rel/TKE/MVPE，重点比较 26,240 / 27,880 / 30,340 等 checkpoint。 | 10k 远未收敛；约 22k 后进入平台/振荡 | 30.9k 完整 validation curve | [30.9k Validation 曲线](CHATGPT_HANDOFF_P0A_N2_VALIDATION_30900.md) | 是 |
| **低学习率二阶段收口（Stage-B + extra Rel）** | 在已经较强的 integrated backbone 上做**同一模型连续训练**：Stage A 用 Dense-All + P0-A + MF-CNO + N2 + vorticity，从 update 1→30,000，LR=`1e-5`；Stage B 不换数据、不换模型，从 @30k optimizer state 继续到 35k，把 LR 降到 `3e-6`，并在原 N2 的 Rel 权重 `0.027514` 之外**再加一个 `0.027514*Rel-L2`**，即 Stage-B 的 Rel 总权重变为 `0.055028`。评估点为 31k / 32.5k / 35k，当前 sweet spot @32.5k。这个阶段本质上是“主训练学总体动力学，低 LR 阶段强化相对误差收口”。 | 明确有效，尤其显著改善 Rel/MVPE | 50/16 matched continuation | [SOTA-V2 50/16 实验](../sota迭代/reviews/sota_v2_integrated_50_16_20260915/README.md) · [Stage-A/B 实现](../../tools/realpde_sota_v2_integrated.py) | 是 |
| **自适应不确定性区间 / SPS 校准** | **完全冻结 point predictor，不改预测值，只学习区间宽度。** 对每个 Future20 的位置/时刻，uncertainty head 输入 15 通道：Past 输入原始 3 通道 + 由 frozen prediction 构造的 12 通道 flow features（预测 `u/v`、speed、energy proxy、`du/dt,dv/dt`、vorticity、divergence、strain、归一化 x/y/time）。head 是 3D Conv：15→32，2 个 residual Conv3D block，最后输出 `sigma_u/sigma_v`；Train50 canonical 2,052 windows 上用 Gaussian NLL 训练 1,400 updates，AdamW `lr=1e-3, wd=1e-5`。随后 Dev16 上只扫固定 28 组 `floor ∈ {0,0.0025,0.005,0.0075}` × `mult ∈ {0.5,1,1.5,2,2.5,3,4}`，区间为 `pred ± (floor + mult*sigma)`，按 SPS 选最优；pressure interval width 固定 0。 | 明确有效，可在 point prediction 不变时提升 Final | Dev + 多次 Codabench online | [Adaptive SPS 实验](../sota迭代/reviews/sota_v2_adaptive_20260916/README.md) · [训练/校准实现](../../tools/realpde_sota_v2_adaptive.py) · [runtime 实现](../../tools/sota_v2_adaptive_runtime.py) | 是 |
| **时间统计 + 空间梯度信息（Temporal + Spatial）** | 这是先做“**信息是否有价值**”的低成本 probe，不直接改 CNO。Baseline 用 persistence：把 Past20 最后一帧 `u/v` 重复成 Future20；目标改为学习 `真实 Future20 - persistence` 的 residual。每个像素输入先有 last-frame `u/v`；Temporal 再加 Past20 的 `mean_u/v, std_u/v` 和最近两帧差 `delta_u/v`；Spatial 再加最后一帧 `du/dx,du/dy,dv/dx,dv/dy`。四组分别用 Train-only 标准化 + closed-form ridge（`alpha=1e-2`）拟合，Dev 不调参。这个实验回答的是“这些 feature 是否含有未来 residual 的增量信息”，不是证明某种 neural fusion 已成功。 | 数据层面明确含有增量信息 | train-only ridge residual probe | [Feature Incremental Probe](CHATGPT_HANDOFF_FE_INCREMENTAL_PROBE.md) | 尚不能等价为神经 fusion 已验证 |
| **均值 / 脉动分解（Mean / Fluctuation）** | 不再让 CNO 直接输出 Future20 的 `u/v`，而把输出头从 3 通道改成 5 通道：`mean_u_raw, mean_v_raw, fluct_u_raw, fluct_v_raw, p`。重构时，mean 分支先沿 Future20 时间维求均值并广播成恒定 mean field；fluctuation 分支减去自身 Future20 时间均值，强制变成 zero-mean fluctuation；最终 `u/v = mean_field + zero_mean_fluctuation`。Direct checkpoint 初始化时，原 u/v projection 同时复制给 mean 与 fluctuation 两组，pressure 单独复制，确保公平 warm-start；其它 P0-A、N2、LR、batch、split 不变。 | 短中程有明显收益，长程 standalone 不稳定 | matched + long convergence | [MF matched closeout](CHATGPT_HANDOFF_MF_DIRECTION_CLOSEOUT.md) · [MF-CNO 实现](../../tools/realpde_mf01.py) | 组合内保留，独立归因未解决 |
| **涡量辅助监督（Vorticity supervision）** | **推理结构完全不变，只改训练 loss。** 在 N2 基础上，对预测和真值 `u/v` 分别用空间差分计算 `ω = dv/dx - du/dy`，增加 `lambda_vort * MSE(ω_pred, ω_gt)`，当前 `lambda_vort=15.5385751724`。matched 长程实验中 C0 与 V1 使用相同 P0-A、N2、初始化、数据、effective batch=8、AdamW `lr=1e-5`；唯一差别就是这项 vorticity loss，因此它不会增加正式 inference 的输入或网络计算分支。 | 有小幅一致信号，但 standalone 长程 gate 未过 | matched long validation | [Vorticity 长程验证](CHATGPT_HANDOFF_VORTICITY_LOWMEM_FINAL.md) · [SOTA 中的实现](../../tools/realpde_sota_v2_integrated.py) | 组合内保留，独立归因未解决 |
| **局部残差修正 + 缩放（H1 / Point residual + scale）** | 先**冻结 CNO 主干**，只训练一个共享到每个空间点的 local residual MLP。每个点输入共 402 维：CNO 对该点 Future20 的 `u/v` 预测 40 维 + Past20 在该点周围 3×3 LOCAL3 邻域的 `u/v` 历史 360 维 + 归一化像素位置 x/y 2 维；MLP 为 `402→256→256→256→128→40`，GELU，最后一层 zero-init，输出 Future20 的 `Δu/Δv`，最终 `pred = CNO + residual`。H1 仅用 UV-MSE、AdamW `lr=1e-4`、batch=8 训练 1,500 updates。由于原始 residual 会明显伤 TKE，随后**不再训练网络**，只在 Train 上扫描全局 `alpha=0.0..1.0`（步长 0.1），计算 `pred=CNO+alpha*residual`；先约束 TKE degradation ≤5%，再选 Rel-L2 最优，得到 `alpha*=0.5`，最后一次性到 Dev 验证。 | Rel/MVPE 很强，但 TKE 风险明显 | frozen-CNO matched offline | [H1 Scale 实验](CHATGPT_HANDOFF_HYBRID_CNO_POINT_H1_SCALE.md) · [H1 runner](../../tools/realpde_hybrid_cno_point_h1_runner.py) · [scale probe](../../tools/realpde_hybrid_cno_point_h1_scale_probe.py) | 有条件有效，暂未进入线上 SOTA |
